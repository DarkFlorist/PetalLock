...
contract PetalLock {
	function makeImmutable(address originalOwner, string[] memory labels, bytes32[] memory subdomainRouteNodes, bytes memory contenthash) private {
		...
		for (uint256 i = 0; i < labels.length; i++) {
			...
			if (!ensRegistry.recordExists(node)) {
				...
				if (i == finalChildIndex) {
					...
					ensTokenWrapper.setSubnodeRecord(parentNameHash, labels[i], address(this), ENS_PUBLIC_RESOLVER, 0, parentFusesToBurn, parentExpiry);
				}
				...
				if (i == 0) {
					...
					if (newFuses & parentFusesToBurn != parentFusesToBurn) {
						(,, uint64 newParentExpiry) = ensTokenWrapper.getData(uint256(subdomainRouteNodes[i - 1]));
						ensTokenWrapper.setChildFuses(subdomainRouteNodes[i - 1], keccak256(abi.encodePacked(labels[i])), parentFusesToBurn, newParentExpiry);
					}
				}
			}
		}
		...
		(,, uint64 newParentExpiry2) = ensTokenWrapper.getData(uint256(subdomainRouteNodes[finalChildIndex - 1]));
		ensTokenWrapper.setChildFuses(subdomainRouteNodes[finalChildIndex - 1], keccak256(abi.encodePacked(labels[finalChildIndex])), childFusesToBurn, newParentExpiry2);
		
		// burn child owner
		ensTokenWrapper.safeTransferFrom(address(this), burnAddress, uint256(subdomainRouteNodes[finalChildIndex]), 1, bytes(''));
		
		// return rest of the tokens
