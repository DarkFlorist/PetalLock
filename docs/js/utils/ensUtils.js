...
export const getRequiredFuses = (domainIndex, domainInfos) => {
...
export const areRequiredFusesBurnt = (domainIndex, domainInfos) => {
...
export const getPetalLockUseTransaction = (petalLockAddress, accountAddress, subdomainRouteNames, ownedTokens, contentHash, resolutionAddress) => {
    ...
    if (subdomainRouteNodes[0] === undefined)
        throw new Error('Not a valid ENS sub domain');
    ...
    for (let i = 0; i < labels.length; i++) {
        ...
        if (labelAtIndex === undefined)
            throw new Error('missing label at index');
        if (nodeAtIndex === undefined)
            throw new Error('missing node at index');
