import { createWalletClient, custom, encodeAbiParameters, keccak256, publicActions, toHex } from 'viem';
...
export function getSubstringAfterFirstPoint(input) {
...
export const getDomainInfo = async (account, nameHash, label, token) => {
...
export const parentFuseToBurn = 'Cannot Unwrap Name';
export const doWeNeedToBurnParentFuses = (parentInfo) => {
    if (!parentInfo.isWrapped)
        return true;
    return !parentInfo.fuses.includes(parentFuseToBurn);
};
export const burnParentFuses = async (account, parentInfo) => {
    if (!doWeNeedToBurnParentFuses(parentInfo))
        return undefined;
    const client = createClient(account);
...
export const mandatoryChildFusesToBurn = ['Parent Domain Cannot Control'];
export const childFusesToBurn = ['Cannot Unwrap Name', 'Cannot Burn Fuses', 'Cannot Set Resolver', 'Cannot Set Time To Live', 'Cannot Create Subdomain', 'Parent Domain Cannot Control', 'Cannot Approve', 'Can Extend Expiry'];
export const doWeNeedToBurnChildFuses = (childInfo) => {
    if (!childInfo.isWrapped)
        return true;
    for (const requiredFuse of mandatoryChildFusesToBurn) {
        if (!childInfo.fuses.includes(requiredFuse))
            return true;
    }
    return false;
};
export const burnChildFuses = async (account, ensLabel, childInfo, parentInfo) => {
    const client = createClient(account);
...
export const wrapDomain = async (account, domainInfo, subdomain) => {
    ...
    if (subdomain) {
        ...
        const requestHash2 = await client.writeContract({
            ...
            abi: ENS_WRAPPER_ABI,
            functionName: 'wrap',
            args: [decodeEthereumNameServiceString(domainInfo.label), account, ENS_PUBLIC_RESOLVER]
        });
...
const burnAddresses = ['0xdeaDDeADDEaDdeaDdEAddEADDEAdDeadDEADDEaD', '0x000000000000000000000000000000000000dEaD', '0x0000000000000000000000000000000000000000'];
export const isChildOwnershipBurned = (childInfo) => {
    return burnAddresses.map((b) => BigInt(b)).includes(BigInt(childInfo.owner)) && childInfo.isWrapped;
};
export const transferChildOwnershipAway = async (account, childInfo) => {
    if (isChildOwnershipBurned(childInfo))
        return undefined;
    const client = createClient(account);
    const requestHash = await client.writeContract({
        chain: mainnet,
...
export const getRightSigningAddress = (transaction, childInfo, parentInfo) => {
    switch (transaction) {
