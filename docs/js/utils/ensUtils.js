import { createPublicClient, createWalletClient, custom, encodeAbiParameters, getAddress, getContractAddress, http, isAddress, labelhash, namehash, numberToBytes, publicActions } from 'viem';
...
export const extractENSFuses = (uint) => {
    if (uint === CAN_DO_EVERYTHING)
...
export const fuseNamesToUint = (names) => {
    let result = 0;
...
export const requestAccounts = async () => {
    if (window.ethereum === undefined)
        throw new Error('no window.ethereum injected');
...
const createReadClient = (accountAddress) => {
...
const createWriteClient = (accountAddress) => {
    if (window.ethereum === undefined)
        throw new Error('no window.ethereum injected');
...
export const getChainId = async (accountAddress) => {
    return await createWriteClient(accountAddress).getChainId();
};
const getDomainInfo = async (accountAddress, nameHash, label, parentDomain, token, subDomain) => {
...
export const getDomainInfos = async (accountAddress, name) => {
...
export const getRequiredFusesWithoutApproval = (domainIndex, domainInfos) => {
    return getRequiredFusesWithApproval(domainIndex, domainInfos).filter((fuse) => fuse !== 'Cannot Approve' && fuse !== 'Can Extend Expiry');
};
export const getRequiredFusesWithApproval = (domainIndex, domainInfos) => {
    if (domainInfos.length === 1)
        return SINGLE_DOMAIN_FUSES;
    if (domainIndex === 0 && domainInfos.length > 1)
        return TOP_PARENT_FUSES;
    if (domainIndex === domainInfos.length - 1)
        return FINAL_CHILD_FUSES;
    return MID_PARENT_FUSES;
};
export const areRequiredFusesBurntWithoutApproval = (domainIndex, domainInfos) => {
    ...
    for (const requiredFuse of requiredFuses) {
        if (!domainInfo.fuses.includes(requiredFuse))
            return false;
    }
    return true;
};
export const isValidEnsSubDomain = (subdomain) => {
...
export const isChildOwnershipOwnedByOpenRenewManager = (childInfo) => {
    return BigInt(getOpenRenewalManagerAddress()) === BigInt(childInfo.owner) && childInfo.isWrapped;
};
export const isChildOwnershipGivenAway = (childInfo) => {
...
export async function ensureProxyDeployerDeployed(accountAddress) {
    const wallet = createWriteClient(accountAddress);
    const deployerBytecode = await wallet.getCode({ address: proxyDeployerAddress });
    if (deployerBytecode === '0x60003681823780368234f58015156014578182fd5b80825250506014600cf3')
        return;
...
export function getPetalLockAddress() {
    const bytecode = `0x${petalLockContractArtifact.contracts['PetalLock.sol'].PetalLock.evm.bytecode.object}`;
...
export function getOpenRenewalManagerAddress() {
    const bytecode = `0x${petalLockContractArtifact.contracts['OpenRenewalManager.sol'].OpenRenewalManager.evm.bytecode.object}`;
    return getContractAddress({ bytecode, from: proxyDeployerAddress, opcode: 'CREATE2', salt: numberToBytes(0) });
}
const isOpenRenewalManagerDeployed = async (accountAddress) => {
    const wallet = createReadClient(accountAddress);
    const expectedDeployedBytecode = `0x${petalLockContractArtifact.contracts['OpenRenewalManager.sol'].OpenRenewalManager.evm.deployedBytecode.object}`;
...
const isPetalLockDeployed = async (accountAddress) => {
    const wallet = createReadClient(accountAddress);
    const expectedDeployedBytecode = `0x${petalLockContractArtifact.contracts['PetalLock.sol'].PetalLock.evm.deployedBytecode.object}`;
    const address = getPetalLockAddress();
    const deployedBytecode = await wallet.getCode({ address });
    return deployedBytecode === expectedDeployedBytecode;
};
export const isPetalLockAndOpenRenewalManagerDeployed = async (accountAddress) => {
...
export const deployPetalLockTransaction = () => {
    const bytecode = `0x${petalLockContractArtifact.contracts['PetalLock.sol'].PetalLock.evm.bytecode.object}`;
    return { to: proxyDeployerAddress, data: bytecode };
};
export const deployOpenRenewalManagerTransaction = () => {
    const bytecode = `0x${petalLockContractArtifact.contracts['OpenRenewalManager.sol'].OpenRenewalManager.evm.bytecode.object}`;
    return { to: proxyDeployerAddress, data: bytecode };
};
export const deployPetalLockAndRenewalManager = async (accountAddress) => {
    ...
    if (openRenewalManagerDeployed && petalLockDeployed)
...
export const getPetalLockUseTransaction = (petalLockAddress, accountAddress, subdomainRouteNames, ownedTokens, contentHash, resolutionAddress) => {
    const labels = subdomainRouteNames.map((p) => {
        const [label] = p.split('.');
        if (label === undefined)
            throw new Error('Not a valid ENS sub domain');
        return label;
    });
    const encodedContentHash = contentHash === '' ? '0x' : tryEncodeContentHash(contentHash);
    if (encodedContentHash === undefined)
        throw new Error('Unable to decode content hash');
    if (resolutionAddress.length > 0 && !isAddress(resolutionAddress, { strict: true }))
        throw new Error('Resolution address is not valid');
...
export const callPetalLock = async (accountAddress, domainInfos, contentHash, resolutionAddress) => {
    const client = createWriteClient(accountAddress);
...
export const renewDomainByYear = async (accountAddress, extendYear, domainInfos) => {
    const client = createWriteClient(accountAddress);
    const petalLockAddress = getPetalLockAddress();
    ...
    const totalRentCost = (await Promise.all(domainInfos.map(async (domain) => domain.fuses.includes('Is .eth domain') ? await client.readContract({
        address: ENS_ETH_REGISTRAR_CONTROLLER,
        abi: ENS_REGISTRAR_CONTROLLER_ABI,
        functionName: 'rentPrice',
        args: [domain.subDomain, extendSeconds]
    }) : { base: 0n, premium: 0n }))).reduce((agg, current) => agg + current.base, 0n);
...
export const renewDomainToMax = async (accountAddress, domainInfos) => {
    const client = createWriteClient(accountAddress);
    const petalLockAddress = getPetalLockAddress();
...
//# sourceMappingURL=ensUtils.js.map