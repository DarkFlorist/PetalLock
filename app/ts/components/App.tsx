...
import { requestAccounts, AccountAddress, DomainInfo, isValidEnsSubDomain, getDomainInfo, getSubstringAfterFirstPoint, doWeNeedToBurnParentFuses, doWeNeedToBurnChildFuses, isChildOwnershipBurned, burnParentFuses, burnChildFuses, transferChildOwnershipAway } from '../utils.js'
...
const WalletComponent = ({ account }: WalletComponentProps) => {
	...
	console.log('render', account.value)
...
export function App() {
	...
	return <main>
		...
		{ childDomainInfo.value === undefined || parentDomainInfo.value === undefined || account.value === undefined? <></> : <>
			...
			{ childDomainInfo.value.isWrapped && BigInt(childDomainInfo.value.owner) !== BigInt(account.value) ? <p style = 'color: #b43c42'>{ `You are not owner of ${ childDomainInfo.value.label }, its owned by ${ childDomainInfo.value.owner }` }</p> : <></> }
			{ parentDomainInfo.value.isWrapped && BigInt(parentDomainInfo.value.owner) !== BigInt(account.value) ? <p style = 'color: #b43c42'>{ `You are not owner of ${ parentDomainInfo.value.label }, its owned by ${ parentDomainInfo.value.owner }` }</p> : <></> }
