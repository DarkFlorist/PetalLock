...
const rpc = 'https://geth.dark.florist';
const petalLockAddress = 0x93741n;
const subdomainRouteNames2 = ['darkflorist.eth', 'immutable.darkflorist.eth'];
const subdomainRouteNames3 = ['darkflorist.eth', 'midname.darkflorist.eth', 'immutable.midname.darkflorist.eth'];
const subdomainRouteNames3_another = ['darkflorist.eth', 'midname.darkflorist.eth', 'immutable2.midname.darkflorist.eth'];
...
const makeImmutableDomain = (routenames, ownedTokens) => {
    const tx = getPetalLockUseTransaction(addressString(petalLockAddress), addressString(ownAddress), [...routenames], [...ownedTokens], testContentHash, addressString(ownAddress));
    return {
        from: ownAddress,
...
const ethSimulateTransactions = async (rpc, transactions) => {
...
const runTests = async () => {
    ...
    const testOpenRenewalManagerAddressIsConstant = async () => {
        ...
        if (calculatedAddress !== openRenewalManagerAddress)
            throw new Error(`The Address of Open Renewal Manager has changed to ${addressString(calculatedAddress)}.`);
    };
    ...
    const testWeHaveTheOrignalToken = async () => {
        ...
        if (bytesToUnsigned(result[0]?.calls[1]?.returnData) !== 1n)
            throw new Error('we lost the token');
    };
    const testThatFusesAreCorrect = async () => {
        ...
        checkSuccess(result);
        if (result[0]?.calls[2]?.returnData === undefined || result[0]?.calls[3]?.returnData === undefined)
            throw new Error('no results for calls');
        ...
        if (!new Set(TOP_PARENT_FUSES).isSubsetOf(new Set(parentFuses)))
            throw new Error(`not correct parent fuses: ${Array.from(parentFuses).join(',')}, Expected: ${TOP_PARENT_FUSES.join(',')}`);
        ...
        if (!new Set(FINAL_CHILD_FUSES).isSubsetOf(childFuses))
            throw new Error('not correct child fuses');
    };
    ...
    const testThatFusesAreCorrect3Part = async () => {
        ...
        if (!new Set(TOP_PARENT_FUSES).isSubsetOf(parentFuses1))
            throw new Error('not correct parent1 fuses');
        // parent 2
        ...
        if (!new Set(MID_PARENT_FUSES).isSubsetOf(parentFuses2))
            throw new Error('not correct parent2 fuses');
        // child
        const childData = decodeFunctionResult({ abi: ENS_WRAPPER_ABI, functionName: 'getData', data: dataStringWith0xStart(result[0]?.calls[3]?.returnData) });
        const childFuses = new Set(extractENSFuses(BigInt(childData[1])));
        if (!new Set(FINAL_CHILD_FUSES).isSubsetOf(childFuses))
        ...
        if (bytesToUnsigned(result[0]?.calls[4]?.returnData) !== 1n)
            throw new Error('we lost the token 1');
        // parent 2 ownership. We should own this
        if (bytesToUnsigned(result[0]?.calls[5]?.returnData) !== 1n)
            throw new Error('we lost the token 2');
        // child ownership
        if (bytesToUnsigned(result[0]?.calls[6]?.returnData) !== 0n)
            throw new Error('we didnt lose the child');
        if (bytesToUnsigned(result[0]?.calls[7]?.returnData) !== 1n)
            throw new Error('open renewal manager does not have the child');
    };
    const testContentHashIsSet = async () => {
        ...
        if (priorContentHash === afterContentHash)
            throw new Error('content has was not set');
        if (decodeContentHash(afterContentHash) !== testContentHash)
            throw new Error('wrong content hash');
        ...
        if (BigInt(priorAddr) === BigInt(afterAddr))
            throw new Error('addr was not set');
        if (BigInt(afterAddr) !== ownAddress)
            throw new Error('addr is not correct');
    };
    const testThatFusesAreCorrect3PartAgain = async () => {
        ...
        if (!new Set(FINAL_CHILD_FUSES).isSubsetOf(childFuses))
            throw new Error('not correct child fuses');
        if (bytesToUnsigned(result[0]?.calls[3]?.returnData) !== 1n)
            throw new Error('open renewal manager does not have the child');
    };
    ...
    const checkApprovals = async () => {
        ...
        checkSuccess(result);
        if (result[0]?.calls[1]?.returnData === undefined || result[0]?.calls[2]?.returnData === undefined || result[0]?.calls[3]?.returnData === undefined)
            throw new Error('fff');
        const renewalManager = getOpenRenewalManagerAddress();
        if (BigInt(dataStringWith0xStart(result[0]?.calls[1]?.returnData)) !== BigInt(renewalManager))
            throw new Error('wrong approval for call 1');
        if (BigInt(dataStringWith0xStart(result[0]?.calls[2]?.returnData)) !== BigInt(renewalManager))
            throw new Error('wrong approval for call 2');
        if (BigInt(dataStringWith0xStart(result[0]?.calls[3]?.returnData)) !== BigInt(renewalManager))
            throw new Error('wrong approval for call 3');
    };
    ...
    const testImmutableDomain = async () => {
        ...
        if (decodeContentHash(contentHash) !== testContentHash)
            throw new Error('wrong content hash');
        ...
        const childFuses = new Set(extractENSFuses(BigInt(childData[1])));
        if (!new Set(SINGLE_DOMAIN_FUSES).isSubsetOf(childFuses))
            throw new Error('not correct child fuses');
        if (bytesToUnsigned(result[0]?.calls[5]?.returnData) !== 1n)
            throw new Error('open renewal manager does not have the child');
    };
    const testAnyoneCanRenewDomainExistingOne = async () => {
        ...
        checkSuccess(result);
        if (result[0]?.calls[1]?.returnData === undefined || result[0]?.calls[3]?.returnData === undefined)
            throw new Error('no results');
    ...
    const domainOrderWrong = async () => {
        const ownedTokens = [BigInt(namehash(subdomainRouteNames2[0]))];
        const tx = getPetalLockUseTransaction(addressString(petalLockAddress), addressString(ownAddress), [subdomainRouteNames2[1], subdomainRouteNames2[0]], [...ownedTokens], testContentHash, addressString(ownAddress));
        const result = await ethSimulateTransactions(rpc, [{
                from: ownAddress,
                to: BigInt(ENS_NAME_WRAPPER),
                input: stringToUint8Array(encodeFunctionData(tx))
        ...
        if (allSuccess(result))
            throw new Error('transaction succeeded while it should fail');
    };
